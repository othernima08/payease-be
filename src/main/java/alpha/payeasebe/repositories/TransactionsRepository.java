...
public interface TransactionsRepository extends JpaRepository<Transactions, String> {
    ...
    @Query(value = "SELECT u.first_name, u.last_name,t.user_id,  t.amount, tc.type FROM Users u, Transactions t, Transaction_categories tc WHERE t.user_id = u.id AND t.transaction_categories_id = tc.id AND t.already_done = TRUE AND t.id = (SELECT id FROM Transactions WHERE user_id = t.user_id ORDER BY transaction_time DESC LIMIT 1)", nativeQuery = true)
